...
import { subscribeToNetflixData } from "@/lib/firebase-netflix";
...
export default function NetflixAdmin() {
  ...
  useEffect(() => {
    ...
    const unsubscribeAdminConfig = subscribeToNetflixData("/admin_config", (adminConfig) => {
      setData((prev: any) => ({ ...prev, adminConfig }));
    });
    
    const unsubscribeSlots = subscribeToNetflixData("/settings/slots", (slots) => {
      setData((prev: any) => ({ ...prev, slots }));
    });
    
    const unsubscribeCredentials = subscribeToNetflixData("/", (credentials) => {
    ...
    const unsubscribeReferrals = subscribeToNetflixData("/referrals", (referrals) => {
      setData((prev: any) => ({ ...prev, referrals }));
    });
    
    const unsubscribeReferralSettings = subscribeToNetflixData("/referral_settings", (referralSettings) => {
      setData((prev: any) => ({ ...prev, referralSettings }));
    });
    
    const unsubscribeTransactions = subscribeToNetflixData("/transactions", (transactions) => {
      setData((prev: any) => ({ ...prev, transactions }));
    });
    
    const unsubscribeOrderIds = subscribeToNetflixData("/used_order_ids", (usedOrderIds) => {
      setData((prev: any) => ({ ...prev, usedOrderIds }));
    });
    
    const unsubscribeFreeTrialClaims = subscribeToNetflixData("/free_trial_claims", (freeTrialClaims) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="text-lg">Loading Netflix admin panel...</p>
      </div>
    );
  }
      ...
      <main className="container py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="screen" className="space-y-6">
            <ScreenConfigPanel service="netflix" />
          </TabsContent>
